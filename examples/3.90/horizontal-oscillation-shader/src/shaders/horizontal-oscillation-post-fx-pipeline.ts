...
void main(void) {
  ...
  // Create a sine wave that varies along the vertical (y) axis of the texture.
  // - outTexCoord.y: The vertical position of the current pixel.
  // - u_frequency: Controls how many wave cycles appear vertically.
  // - u_time * u_speed: Animates the wave over time.
  float wave = sin(outTexCoord.y * u_frequency + u_time * u_speed);

  // Calculate the horizontal offset for the current pixel.
  // - wave: The sine wave value (-1.0 to 1.0).
  // - u_amplitude: Controls the maximum distance the pixels are pushed left or right.
  float offset = wave * u_amplitude;

  // Create a new texture coordinate by applying the horizontal offset.
...
/**
 * A PostFX pipeline that applies a horizontal effect.
 */
