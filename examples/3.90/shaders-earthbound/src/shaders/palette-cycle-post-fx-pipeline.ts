...
export class PaletteCyclePostFxPipeline extends Phaser.Renderer.WebGL.Pipelines.PostFXPipeline {
  ...
  private paletteTexture: Phaser.Renderer.WebGL.Wrappers.WebGLTextureWrapper | null = null;
   ...
   */
  setPalette(textureKey: string): void {
    this.paletteTexture = this.game.textures.getFrame(textureKey).glTexture;
    this.paletteWidth = this.paletteTexture.width;

    // const texture = this.game.textures.get(textureKey);
    // if (texture && texture.source && texture.source.glTexture) {
    //   this.paletteTexture = texture.source.glTexture;
    //   // Automatically set the palette width from the texture's source image.
    //   this.paletteWidth = texture.source.width;
    // } else {
    //   console.warn(`Palette texture not found or not ready: ${textureKey}`);
